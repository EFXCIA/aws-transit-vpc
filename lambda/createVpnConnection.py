...
def updateDynamoDb(tableName, vpcId, vpcCidr, awsRegion):
    ...
    try:
        ...
        logger.info('Updated Subscriber local DynmodDB with vpc-id: {} and '
                    'vpc-cidr: {}'.format(vpcId, vpcCidr))
...
def updateTags(awsRegion, vpcId, oldVpc):
    ...
    try:
        ...
        configReason = ('Vpc-CIDR Conflicts with {}: '
                        '{}').format(oldVpc['VpcId'], oldVpc['Region'])
        tags = [
            {'Key': 'ConfigStatus', 'Value': 'Vpn-Failed'},
            {'Key': 'ConfigReason', 'Value': configReason}
        ]
        ec2Connection.create_tags(Resources=[vpcId], Tags=tags)
        logger.info('Successfully Updated VPC-Failed tags to '
                    'VPCID: {}'.format(vpcId))
        sys.exit(0)
    except Exception as e:
        logger.info('Updating VPC-Failed tags failed, '
                    'Error: {}'.format(str(e)))
        sys.exit(0)
