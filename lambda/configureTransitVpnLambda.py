...
def updateVpcTable(tableName, data, status):
    '''Updates the Transit VpcTable with VpcId, Node1VpnId, Node2VpnId, Region,
    IpSegment and CurrentStatus
    '''
    try:
        # VpcId is the primary key for VpcTable
        table = dynamodb.Table(tableName)
        table.update_item(
            Key={'VpcId': data['VpcId']},
            AttributeUpdates={
                'CurrentStatus': {
                    'Value': status,
                    'Action': 'PUT'
                ...
                'IpSegment': {
                    'Value': data['IpSegment'],
                    'Action': 'PUT'
                }
            }
        )
    except Exception as e:
        logger.error('Updating Transit VpcTalbe is Failed, '
                     'Error: {}'.format(str(e)))
...
def updateBgpTunnelIpPool(bgpTableName, ipSegment):
    ...
    J.G. fixed a glaring bug here:
    - "table" was assigned but never used
    - "tableConn.update_item" was executed on an undefined variable.
    change "tableConn" to just "table"
    '''
    try:
        table = dynamodb.Table(bgpTableName)
        # Update BgpTunnelIpPool table Attribute "Available"="YES"
        table.update_item(
            Key={'IpSegment': ipSegment},
            AttributeUpdates={'Available': {'Value': 'YES', 'Action': 'PUT'}}
        )
        logger.info('Successfully Updated BgpTunnleIpPool Table attribute '
                    'Available=YES')
    except Exception as e:
        logger.error('Update BgpTunnelIpPool is failed, '
                     'Error: {}'.format(str(e)))


def updatePaGroup(paGroupTableName, paGroupName, value):
    '''Updates Transit PaGroupInfo table attribute VpcCount to either +1
    or -1 based on the value paramater passed to the function
    '''
    try:
        ...
        if response:
            if response[0]['VpcCount'] > 0:
                ...
                logger.info('Successfully Updated PaGroupInfoTable '
                            'decremented VpcCount by 1')
    except Exception as e:
        logger.error('updatePaGroupInfo() Table is failed, '
                     'Error: {}'.format(str(e)))
...
def getPaGroupInfo(tableName, paGroup):
    ...
    try:
        table = dynamodb.Table(tableName)
        resp = table.query(
            KeyConditionExpression=Key('PaGroupName').eq(paGroup)
        )
        response = resp.get('Items')
        return response[0]
    except Exception as e:
        logger.error('Fetch Item from PaGroupInfo failed, '
                     'Error: {}'.format(str(e)))
            ...
            for item in response['Items']:
                if 'VpcId' in item:
                    if item['VgwAsn'] != vgwAsn:
                        ...
                        logger.info('VgwAsn are mis-matched, because user '
                                    'created the VGW and attached it to VPC, '
                                    'hence using the same VGW, updating '
                                    'previously allocated VgwAsn: {} with '
                                    'InUse=NO in Transit VgwAsn '
                                    'table'.format(item['VgwAsn']))
                        return
    ...
    if config:
        ...
        if paGroupInfo:
            ...
            if paVpnStatus:
                ...
                updateVgwAsn(config['TransitVgwAsn'],
                             event['VpcId'],
                             event['VgwAsn'])
                data = {
                ...
                logger.info('Publishing message to Subscriber SNS with '
                            'data: {}'.format(data))
                publishToSns(event['SubscriberSnsArn'],
                             data,
                ...
                updateBgpTunnelIpPool(config['TransitBgpTunnelIpPool'],
                                      event['IpSegment'])
                updateVpcTable(config['TransitVpcTable'],
                               event,
                               'Failed')
                updateVgwAsn(config['TransitVgwAsn'],
                             event['VpcId'],
