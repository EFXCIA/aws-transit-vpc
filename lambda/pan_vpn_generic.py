...
def getVpnConfigurationAndUploadToS3(vpnId, region, bucketName):
    ...
    s3 = boto3.resource('s3')
    try:
        vpnConfig = client.describe_vpn_connections(VpnConnectionIds=[vpnId])['VpnConnections'][0]['CustomerGatewayConfiguration']
    except:
...
def paGroupSetupPaPeers(api_key, newPaGroup, paGroupList):
    ...
    if not failed:
        ...
        for paGroup in paGroupList:
            panCommit(paGroup['N1Mgmt'], api_key, message="Create bgp peer from Active node of PaGroup {0} to active nodes of other PA groups".format(newPaGroup['PaGroupName']))
            panCommit(paGroup['N2Mgmt'], api_key, message="Create bgp peer from Passive node of PaGroup {0} to active nodes of other PA groups".format(newPaGroup['PaGroupName']))
        return True
    else:
...
def parseStackOutput(stackName, region):
    ...
    if status['Stacks'][0]['StackStatus'] == "CREATE_IN_PROGRESS":
        return "Wait"
    elif status['Stacks'][0]['StackStatus'] == "CREATE_COMPLETE" or status['Stacks'][0]['StackStatus'] == "UPDATE_COMPLETE":
        outputs = {}
...
def paGroupConfigureVpn(api_key, paGroup, vpnConfigBucket, N1VpnId, N2VpnId, ikeProfile="default",
    ...
    N2VpnStatus = paConfigureVpn(paGroup['N2Mgmt'], api_key, vpnN2Conf, peerGroup, ikeProfile, ipsecProfile, pa_dmz_inf, virtualRouter, zone)
...
def paGroupDelPaPeers(api_key, newPaGroup, paGroupList):
    ...
    for paGroup in paGroupList:
        ...
        peerGroup = "Active"
        deleteFromPeerGroup(newPaGroup['N1Mgmt'], api_key, peerGroup, "-".join([newPaGroup['PaGroupName'], "N1", paGroup['PaGroupName'], "N1"]), virtualRouter="default")

        # Remove paGroup.N1 <--> newPaGroup.N1
        deleteFromPeerGroup(paGroup['N1Mgmt'], api_key, peerGroup, "-".join([paGroup['PaGroupName'], "N1", newPaGroup['PaGroupName'], "N1"]), virtualRouter="default")

        # Remove newPaGroup.N2 <--> paGroup.N2
        peerGroup = "Passive"
        deleteFromPeerGroup(newPaGroup['N2Mgmt'], api_key, peerGroup, "-".join([newPaGroup['PaGroupName'], "N2", paGroup['PaGroupName'], "N2"]), virtualRouter="default")

        ## Remove paGroup.N2 <--> newPaGroup.N2
        deleteFromPeerGroup(paGroup['N2Mgmt'], api_key, peerGroup,"-".join([paGroup['PaGroupName'], "N2", newPaGroup['PaGroupName'], "N2"]), virtualRouter="default")

    if not failed:
        # call commit on all nodes
        for paGroup in paGroupList:
            panCommit(paGroup['N1Mgmt'], api_key, message="Remove bgp peer entries related to paGroup {0}".format(newPaGroup['PaGroupName']))
            panCommit(paGroup['N2Mgmt'], api_key, message="Remove bgp peer entries related to paGroup {0}".format(newPaGroup['PaGroupName']))
        return True
    else:
        return False
