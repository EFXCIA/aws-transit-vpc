...
from commonLambdaFunctions import sendToQueue
import sys
...
def checkVpcCidrConflicts(vpcCidr, tableName):
    ...
    If yes send notification back to Subscriber SNS that Vpn configuration
    failed create log VPCCIDR conflict “New VPCID, NewVPC CIDR, Existing VPCID,
    Existing VPCCIDR
    '''
    try:
        table = dynamodb.Table(tableName)
        resp = table.scan(
            FilterExpression=Attr('VpcCidr').eq(vpcCidr)
        )
        ...
        logger.error('Checking of CIDR confilcts failed, '
                     'Error: {}'.format(str(e)))
...
def getAvailableBgpTunnelIpPool(tableName, vpcId, paGroupName):
    ...
    finds any items with this condition returns that item otherwise returns
    false calls the updateBgpTunnleIpPool function to update the attribute
    'Available' to NO
    '''
    try:
        logger.info('Fetching BgpTunnelIpPool data with filter '
                    'status=available')
        table = dynamodb.Table(tableName)
        resp = table.scan(
        ...
        if response:
            ...
            updateBgpTunnelIpPool(response[0]['IpSegment'], table, vpcId,
                                  paGroupName)
            return response[0]
        else:
            return False
    except Exception as e:
        logger.error('getAvailableBgpTunnelIpPool failed, '
                     'Error: {}'.format(str(e)))


def getAvailablePaGroup(tableName, maxCount):
    '''Scans the PaGroupInfo table with attributes 'InUse=YES' and 'VpcCount'
    less than MaxPaGroupCapacity, if it finds an items it will return that
    item, otherwise otherwise: it scans the table with attribute 'InUse=NO',
    if it finds an item it will return othrwise returns False Calls
    updatePaGroup() function to update the 'InUse' to YES and increment the
    VpcCount by +1
    '''
    try:
        table = dynamodb.Table(tableName)
        resp = table.scan(
            FilterExpression=Attr('InUse').eq('YES') &
            Attr('VpcCount').lt(maxCount)
        )
        response = resp.get('Items')

        logger.info('PaGroup Info scan result with Fileter InUse=YES and '
                    'VpcCount < {} is: {}'.format(maxCount, response))
        if response:
            ...
            for item in response:
                if 'N1Eip' in item:
                    ...
                    return False
            logger.info('Returing the Pa Group which has nearest '
                        'capacity, PA-Group '
                        'Name: {}'.format(paGroupToReturn['PaGroupName']))
            ...
            resp = table.scan(
                FilterExpression=Attr('InUse').eq('NO')
            )
            response = resp.get('Items')
            if response:
                for group in response:
                    if 'N1Eip' in group:
                        # Update PaGroupInfo Table InUse="Yes" and increment
                        # VpcCount+1
                        logger.info('Returing the PA-Group '
                                    'Name: {}'.format(group['PaGroupName']))
...
def getAvailableVgwAsn(tableName, data):
    '''Scans the VgwAsn table with attribute 'InUse=NO', if it finds an item
    it will return that item, otherwise exit from the process Calls
    updateVgwAnsTable() function to update the 'InUse' to YES and VpcId and
    VpcCidr
    '''
    try:
        table = dynamodb.Table(tableName)
        ...
        r = table.scan(
            FilterExpression=Attr('VpcId').eq(data['VpcId'])
        )
        res = r.get('Items')
        if res:
            logger.info('The VPC: {} is already assigned with '
                        'VgwAsn: {}'.format(data['VpcId'],
                                            res['Items'][0]['VgwAsn']))
            return res[0]['VgwAsn']
        resp = table.scan(
            FilterExpression=Attr('InUse').eq('NO')
        ...
        if response:
            ...
            updateVgwAsnTable(response[0]['VgwAsn'], data, table)
            return response[0]['VgwAsn']
        else:
            logger.error('VgwAsn numbers are exhausted, so Pleas add some '
                         'more ASN numbers to VgwAsn Table')
            sys.exit(0)
    except Exception as e:
