...
def updateAssumeRole(roleName, accountNumbers):
    '''Updates Transit Assume Role with Subscriber Account numbers that are
    passed while creating/updating the initializeTranstiAccount CFT
    '''
    ...
    if len(accountNumbers.split(',')) > 1:
        ...
        iam_conn.update_assume_role_policy(RoleName=roleName,
                                           PolicyDocument=policyDoc)
        print('{} is updated with new Account '
              'numbers, Response: {}'.format(roleName, policyDoc))
    else:
...
def updateBgpTunnelIpPoolTable(tableName):
    '''Updates BgpTunnelIpPool table with  attributes IpSegment, N1T1, N1T2,
    N2T1, N2T2 and Available=YES
    '''
    try:
        ...
        for subnet_28 in tunnelCidrRange.subnets(new_prefix=28):
            if count < 200:
                if not str(subnet_28) in exceptionList:
                    ...
                    count += 1
        print ('Updating {} with 200 entries Done, the last item is: '
               '169.254.12.208/28'.format(tableName))
...
def updateTransitConfig(tableName, data):
    ...
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table(tableName)
        # Removing SubscriberAccounts key from data to avoid AttributeValue
        # may not contain an empty string error
...
def updatePaGroupInfo(tableName):
    '''Updates Transit PaGroupInfo table with attributes PaGroupName, N1Asn
    and N2Asn (from 64713-64913), InUse=NO and VpcCount=0
    '''
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table(tableName)
        # Update PaGroupInfo table with PaGroup{id}, N1Asn and N2Asn
        # with range 64713-65113
        asnStart = 64713
...
def updateVgwAsn(tableName):
    '''Updates the Transit VgwAsn table with attributes VgwId from 64512-64612
    and InUse=NO
    '''
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table(tableName)
        # Update VgwAsn Table with Privat ASN numbers between
        # 64512-64612 -> for 100 VPC 100 VGW
...
def lambda_handler(event, context):
    ...
    if event['RequestType'] == 'Create':
        ...
        bucket.delete()
        print('Successully Deleted S3 Objects and the '
        ...
        try:
            ...
            ifaces = ec2_conn.describe_network_interfaces(Filters=filters)
            interfaces = ifaces.get('NetworkInterfaces')
            if interfaces:
                for interface in interfaces:
                    print('Detaching Network Interface: {}'.format(
                        interface['NetworkInterfaceId']
                    ))
                    ec2_conn.detach_network_interface(
                        AttachmentId=interface['Attachment']['AttachmentId']
                    )
                    print('Detached Network Interface: {}'.format(
                        interface['NetworkInterfaceId'])
                    )
            ...
            if interfaces:
                for interface in interfaces:
                    print('Deleting Network Interface: {}'.format(
                        interface['NetworkInterfaceId'])
                    )
                    ec2_conn.delete_network_interface(
                        NetworkInterfaceId=interface['NetworkInterfaceId']
                    )
                    print('Deleted Network Interface: {}'.format(
                        interface['NetworkInterfaceId'])
                    )
            ...
            print('Error while deleting the Network Interfaces|'
                  'TrustedSg|MgmtSubnets|Vpc. Error: {}'.format(str(e)))
            cfnresponse.send(event, context, cfnresponse.SUCCESS, responseData,
                             'CustomResourcePhysicalID')
        cfnresponse.send(event, context, cfnresponse.SUCCESS, responseData,
                         'CustomResourcePhysicalID')
