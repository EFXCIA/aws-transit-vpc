...
def deleteItemFromLocalDb(tableName, vpcId):
    ...
    try:
        ...
        resp = table.query(
            KeyConditionExpression=Key('VpcId').eq(vpcId)
        )
        response = resp.get('Items')

        if response:
            table.delete_item(Key={'VpcId': vpcId})
            logger.info('Successfully Deleted Item  with '
                        'vpc-id : {}'.format(vpcId))
            return response[0]
        else:
            logger.info('The Item has been deleted in the Rebalance '
                        'Operation, hence exiting from the process')
            sys.exit(0)
    except Exception as e:
        logger.error('Deleting of Item with vpc-id: {}, '
                     'Error: {}'.format(vpcId, str(e)))
...
def deleteItemFromVpcVpnTable(tableName, vpnId):
    ...
    try:
        ...
        logger.error('Error from deleteItemFromVpcVpnTable is failed, '
                     'Error: {}'.format(str(e)))
...
def deleteVpn(vpnId, region):
    ...
    try:
        ...
        logger.error('Error from deleteOtherVpn is failed, '
                     'Error: {}'.format(str(e)))
        pass
...
def getCgwId(vpnId, awsRegion):
    ...
    try:
        ...
        resp = ec2_conn.describe_vpn_connections(
            VpnConnectionIds=[vpnId]
        )['']
        response = resp.get('VpnConnections')
        if response:
            return response[0]['CustomerGatewayId']
        else:
            return False
    except Exception as e:
...
def deleteCgw(cgwId, awsRegion):
    ...
    try:
        ...
        logger.error('Error from deleteCgw is failed, '
                     'Error: {}'.format(str(e)))
        pass


def updateTags(awsRegion, vpcId):
    '''Updates the tags of VPC with VPN-Deleted, VPN-Removed for keys
    ConfigStatus and ConfigReason respectively
    '''
    try:
        ec2_conn = boto3.client('ec2', region_name=awsRegion)
        tags = [
        ...
        logger.error('Updating VPC-Failed tags failed, '
                     'Error: {}'.format(str(e)))
...
def lambda_handler(event, context):
    ...
    if subscriberConfig:
        ...
        try:
            if cgw1Id:
                deleteCgw(cgw1Id, event['Region'])
            if cgw2Id:
                deleteCgw(cgw2Id, event['Region'])
        except Exception as e:
        ...
        logger.info('Publishing message to TransitSnsArn: {} with '
                    'data: {}'.format(transitSnsTopicArn, event))
